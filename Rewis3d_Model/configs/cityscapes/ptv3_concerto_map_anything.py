...
# Concerto pretrained weights (encoder only, for initializing 3D backbone)
concerto_weight = None
...
hooks = [
    dict(type="ConcertoCheckpointLoader"),  # Load Concerto pretrained weights first
    dict(type="CheckpointLoader"),  # Then load any saved checkpoint for resuming
...
model_3d = dict(
    ...
    backbone=dict(
        type="PT-v3m2",
        in_channels=9,  # coord(3) + color(3) + normal(3), normals will be zeros
        order=("z", "z-trans", "hilbert", "hilbert-trans"),
        stride=(2, 2, 2, 2),
        # Concerto Large encoder configuration
...
data = dict(
    ...
    train=dict(
        ...
        transform=[
            ...
            # Add zero normals for Concerto (expects 9-channel input: coord + color + normal)
            dict(
                type="AddZeroNormal",
                coord_key="student_coord",
                normal_key="student_normal",
            ),
            dict(type="ToTensor"),
            dict(
                type="Collect",
                ...
                # Include normal in feat_keys for 9-channel input
                student_feat_keys=["student_coord", "student_colors", "student_normal"],
                teacher_feat_keys=["teacher_coord", "student_colors", "student_normal"],
            ),
        ],
        transform_2d=[
            dict(type="ToPIL"),
